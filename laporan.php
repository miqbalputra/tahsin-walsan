...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Rekapitulasi Presensi</h2>
        <p class="text-slate-500 text-sm">Monitoring kehadiran dan perkembangan materi</p>
    </div>
...
<div class="space-y-6">
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 items-end">
            ...
            <div>
                <button type="submit"
                    class="w-full bg-slate-800 text-white font-bold py-2.5 rounded-xl hover:bg-slate-900 transition shadow-lg shadow-slate-100 uppercase text-xs">
                    Cari </button>
            </div>
        </form>
    </div>
    ...
    <!-- Feedback Section -->
    <?php if ($message): ?>
        <div class="bg-emerald-50 text-emerald-600 p-4 rounded-xl border border-emerald-100 italic text-sm">
            <?php echo $message; ?>
        </div>
    <?php endif; ?>
    <?php if ($error): ?>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-50 text-sm">
                    <?php if (empty($results)): ?>
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400 italic">Data tidak ditemukan.</td>
                        </tr>
                    <?php endif; ?>
